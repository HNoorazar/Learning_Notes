...
({\bf  \color{red} ID 2059: Player with Longest Streak})
...
The streak ends once a player loses their next match.\sidenote{This was difficult}.
...
\begin{lstlisting}[language=SQL]
WITH match_groups AS (SELECT player_id, match_date, match_result,
        SUM(CASE WHEN match_result = 'L' THEN 1 ELSE 0 END)
             OVER (PARTITION BY player_id ORDER BY match_date) AS loss_group
                      FROM players_results),
    win_streaks AS (SELECT player_id, loss_group, COUNT(*) AS streak_length
                    FROM match_groups
...
FROM win_streaks
GROUP BY player_id
HAVING MAX(streak_length) = (SELECT MAX(streak_length)FROM win_streaks);
\end{lstlisting}
\end{exm}


\begin{exm}\label{exm:10547}
({\bf \color{red} ID 10547: Actor Rating Difference Analysis})
...
If an actor has only one film, return 0 for the difference and 
their only film’s rating for both the average and latest rating fields.
\begin{lstlisting}[language=SQL]
WITH ranked_films AS (SELECT actor_name, film_title, release_date, film_rating,
...
SELECT r.actor_name, 
       ...
       ROUND(CASE WHEN r.film_count = 1 THEN 0 ELSE r.film_rating - a.avg_prev_rating END, 2) AS rating_difference
FROM ranked_films r
...
Retained customers
\begin{lstlisting}[language=SQL]
WITH t AS (SELECT date, ID, LAG(date) OVER (PARTITION BY ID ORDER BY date) AS prev_date
           FROM activity)
SELECT date, COUNT(DISTINCT ID) AS active_users,
  COUNT(DISTINCT CASE WHEN prev_date=DATE(date, '-1 day') THEN ID END) AS retained_u,
  COUNT(DISTINCT CASE WHEN prev_date=DATE(date, '-1 day') THEN ID END)*1.0/COUNT(DISTINCT ID) AS retention
FROM t
GROUP BY date
ORDER BY date;
\end{lstlisting}
...
\begin{lstlisting}[language=SQL]
WITH user_activity AS (SELECT date, ID, LAG(date) OVER (PARTITION BY ID ORDER BY date) AS prev_date
                       FROM activity),
   daily_stats AS (SELECT date, COUNT(DISTINCT ID) AS active_users,
                     COUNT(DISTINCT CASE WHEN prev_date=DATE(date, '-1 day') THEN ID END) AS retained_users
                   FROM user_activity
                   GROUP BY date)
SELECT date, active_users, retained_users, retained_users*1.0/active_users AS retention
FROM daily_stats
ORDER BY date;
\end{lstlisting}
\end{exm}
