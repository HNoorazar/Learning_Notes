...
\begin{lstlisting}[language=SQL]
CREATE TEMPORARY TABLE sandals AS (SELECT * 
                                    FROM Shoes
...
You already see how \texttt{WHERE} is 
used to filter rows:
\begin{lstlisting}[language=SQL]
SELECT col1, col2
FROM table
...
\texttt{productName = 'Tofu' OR productName = ' 'Kombu'}: Kombu will be ignored. 
...
\hline
\end{tabular}
\caption{Logical order of SQL query processing. According to this table, when an alias is created in SELECT statement, it cannot be used in GROUP BY, but it can be used in ORDER BY.}
\end{table}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{00_figures/SQL/SQL_Logic}
\caption{SQL order of execution}
\label{fig:SQL_Logic}
\end{figure}

\subsection{Wild Cards}
The followings are examples of wild cards. One downside to it is that
they are slow to process.
\begin{itemize}[itemsep=1pt]
\item \code{'\%pizza'} ending with pizza
\item \code{'pizza\%'} after the word pizza
\item \code{'\%pizza\%'} grabs things before and after pizza
\item \code{'s\%e'} grabs anything that starts with S and ends with E.
\item \code{'\_pizza'}: Underscore matches with only one character. So, this code
will grab anything like \code{mpizza} or \code{spizza}
\item Not in SQLite: The bracket [] is used to specify a set of characters in a specific location\sidenote{Expand on this}.
\end{itemize}

For example, we use these with \code{LIKE} like so:

\begin{lstlisting}[language=SQL]
WHERE size_ LIKE '%pizza'
...
\subsection{Sorting}
\code{ORDER BY} is used to sort the output. It comes at the end of
code. It can be used by column name or column position.
...
\subsection{Math Operations and Aggregate Functions}