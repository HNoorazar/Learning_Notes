...
\begin{lstlisting}[language=SQL, linewidth=1.62\textwidth]
-- 1-day retention. 
...
is specified, by default, it does running total:
...
------
with rank_t AS (SELECT *, row_number() OVER (PARTITION BY actor_name ORDER BY release_date DESC) as rn
                FROM actor_rating_shift),
last_t AS (SELECT * FROM rank_t WHERE rn = 1),
avg_t AS (SELECT actor_name, AVG(film_rating) AS avg_film
          FROM rank_t
          WHERE rn > 1
          GROUP BY actor_name)
SELECT l.actor_name, l.film_rating, COALESCE(a.avg_film, l.film_rating) AS average_,
          l.film_rating - COALESCE(a.avg_film, l.film_rating) as delta_rate
FROM last_t l LEFT JOIN avg_t a USING(actor_name)
\end{lstlisting}
\end{exm}
...
Compare the total number of comments made by users in each country during December 2019 and January 2020.\sidenote{User 5 has a duplicate row in fb\_comments\_count table. Why? did he make 2 comments on 2019-12-19 and it was not aggregated? or this is a mistake?}
...
\end{itemize}
\begin{lstlisting}[language=SQL, linewidth=1.45\textwidth]
-- Below, c.created_at >= '2019-12-01' is put in the JOIN statement. that way even if 
-- users don't have a comment on a given month, they will be presented.
WITH helper_t AS (SELECT u.country, MONTH(c.created_at) AS cmt_month,
                        COALESCE(SUM(c.number_of_comments), 0) AS cmt_cnt
                 FROM fb_active_users u LEFT JOIN fb_comments_count c ON u.user_id = c.user_id
                            AND c.created_at >= '2019-12-01' AND c.created_at < '2020-02-01'
                WHERE u.country IS NOT NULL
                GROUP BY u.country, MONTH(c.created_at)),
helper_rank AS (SELECT country, cmt_month, cmt_cnt,
...
-- This passes Strata test. but it is incorrect: it does not handle the case in which 
-- a country does not have a comment in a given month! we should have a value 
-- for comment_count for each pair of country and month. so, either use 
-- UNION ALL or introduce zero before ranking. These are above.
...
helper_rank AS (SELECT *,  
                   DENSE_RANK() OVER (PARTITION BY cmt_month ORDER BY cmt_cnt DESC) AS rnk
                FROM helper_t),
pivoted AS (SELECT country, MIN(CASE WHEN cmt_month=12 THEN rnk END) AS dec_rank,
                            MAX(CASE WHEN cmt_month=1  THEN rnk END) AS jan_rank
            FROM helper_rank
            GROUP BY country)
SELECT country
...
\begin{lstlisting}[language=SQL, linewidth=1.45\textwidth]
-- UNION ALL Solution
WITH countries AS (SELECT DISTINCT country
                   FROM fb_active_users),
months AS (SELECT 12 AS cmt_month UNION ALL SELECT 1),
country_month AS (SELECT c.country, m.cmt_month
                  FROM countries c CROSS JOIN months m),
helper_t AS (SELECT cm.country, cm.cmt_month, COALESCE(SUM(cnt.number_of_comments), 0) AS cmt_cnt
             FROM country_month cm LEFT JOIN fb_active_users u ON cm.country = u.country
                       LEFT JOIN fb_comments_count cnt ON cnt.user_id = u.user_id 
                                       AND cnt.created_at >= '2019-12-01'
                                       AND cnt.created_at < '2020-02-01'
                                       AND MONTH(cnt.created_at) = cm.cmt_month
             GROUP BY cm.country, cm.cmt_month),
helper_rank AS (SELECT *, DENSE_RANK() OVER (PARTITION BY cmt_month ORDER BY cmt_cnt DESC) AS rnk
                FROM helper_t),
pivoted AS (SELECT country, MAX(CASE WHEN cmt_month = 12 THEN rnk END) AS dec_rank,
                      MAX(CASE WHEN cmt_month = 1 THEN rnk END) AS jan_rank
            FROM helper_rank
...
\begin{lstlisting}[language=SQL, linewidth=1.4\textwidth]
with t AS (SELECT actor_name, genre, COUNT(*) as frequency, AVG(movie_rating) as avg_rating
           FROM top_actors_rating
           GROUP BY actor_name, genre),
ranked AS (SELECT *, RANK() OVER 
                      (PARTITION BY actor_name ORDER BY frequency DESC, avg_rating DESC) as rnk
           FROM t),
clean_t AS (SELECT *
            FROM ranked
            where rnk=1)
SELECT *
FROM clean_t
-- The reason for having 2 SELECT statements here is that MySQL does 
-- not allow to use IN (SELECT) with one SELECT only.
WHERE avg_rating IN (SELECT avg_rating 
                     FROM (SELECT DISTINCT avg_rating
                          FROM clean_t
...
\begin{exm} ({\bf \color{red}ID 10131: \href{https://platform.stratascratch.com/coding/10131-business-name-lengths?code_type=3}{Business Name Lengths}})\\
...
\noindent ChatGPT wrote this and seems it is working.
I did not know any of these.
\code{TRIM} removes leading and tailing spaces.
\code{REGEXP\_REPLACE(..., ' +', ' ')} replaces multiple spaces by only 1 space.
The \code{'+'} sign is a wild card meaning more of previous character.
What the code is doing is counting number spaces; length of everything (including spaces) minus length of everything with spaces removed.
\begin{lstlisting}[language=SQL, linewidth=1.5\textwidth]
SELECT business_name,
    (
      LENGTH(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(business_name, '[^A-Za-z0-9 ]', ' '), ' +', ' ')))
      - LENGTH(REPLACE(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(
                           business_name, '[^A-Za-z0-9 ]', ' '), ' +', ' ')), ' ', '')) + 1
    ) AS word_count
FROM sf_restaurant_health_violations;
\end{lstlisting}
\end{exm}

\begin{exm} ({\bf \color{red}ID 9776: \href{https://platform.stratascratch.com/coding/9776-common-interests-amongst-users?code_type=3}{Common Interests Amongst Users}})~\idea\idea\idea\marginnote{\idea \idea This has so much going on. Converting and expanding a string from a column into several rows, taking intersection and count of pairwise users}\\
...
\begin{lstlisting}[language=SQL, linewidth=1.6\textwidth]
WITH user_interests AS (SELECT poster, TRIM(jt.interest) AS interest
                        FROM facebook_posts, -- CROSS JOIN: not having anything here is like CROSS JOIN
                             JSON_TABLE(CONCAT('["', 
                     REPLACE(REPLACE(REPLACE(post_keywords, '[', ''),  ']', '' ),  ',', '","'  ), 
                                               '"]'
                                               ), 
                                        '$[*]' COLUMNS (interest VARCHAR(100) PATH '$')
                                        ) AS jt
                        ),
pairwise_matches AS (SELECT a.poster AS user1, b.poster AS user2, a.interest
                     FROM user_interests a JOIN user_interests b ON a.interest=b.interest 
                     AND a.poster < b.poster
                     ),
shared_count_t AS (SELECT user1, user2, COUNT(*) AS shared_count
                 FROM pairwise_matches
                 GROUP BY user1, user2),
user_counts AS (SELECT poster, COUNT(*) AS total_interests
                FROM user_interests
                GROUP BY poster)
SELECT s.user1, s.user2, s.shared_count,
          ROUND(100.0*s.shared_count/(u1.total_interests+u2.total_interests-s.shared_count), 2) 
                              AS overlap_percentage
FROM shared_count_t s JOIN user_counts u1 ON s.user1=u1.poster JOIN user_counts u2 ON s.user2=u2.poster;
\end{lstlisting}
\end{exm}

\begin{exm} ({\bf \color{red}ID 9821: \href{https://platform.stratascratch.com/coding/9821-common-friends-friend?code_type=3}{Common Friends Friend}})\\
Find the number of a user's friends' friend who are also the user's friend. Output the user id along with the count.
\begin{lstlisting}[language=SQL, linewidth=1.6\textwidth]
WITH second_f AS (SELECT g1.user_id, g2.friend_id AS hop_friend
                  FROM google_friends_network g1 JOIN google_friends_network g2 ON g1.friend_id = g2.user_id)
SELECT g.user_id, COUNT(DISTINCT g.friend_id) AS mutual_count
FROM google_friends_network g JOIN second_f sf ON g.user_id = sf.user_id AND g.friend_id = sf.hop_friend
GROUP BY g.user_id
ORDER BY g.user_id;
\end{lstlisting}
\end{exm}
...
\begin{exm} ({\bf \color{red}ID 517: \href{https://platform.stratascratch.com/coding/10572-city-with-most-amenities?code_type=3}{City With Most Amenities}})\\
...
Treat \code{amenities} as a comma-separated list and count each listed entry as-is, even if the same  \code{amenities} appear multiple times within the same property's amenities, count each occurrence (do not deduplicate).

If multiple cities tie for the highest total, return all of those cities. Output the name of the city/cities.


\noindent Both of following solutions can break down if there are extra commas
somewhere in the list of amenities!

\begin{lstlisting}[language=SQL, linewidth=1.4\textwidth]
with all_amenities as (SELECT a.city, TRIM(t.interest) as interest
                       FROM airbnb_search_details a JOIN JSON_TABLE(concat('["',
...
count_per_city AS (SELECT city, count(interest) as amenity_count
                   FROM all_amenities
                   GROUP BY city)
SELECT city
FROM count_per_city 
WHERE amenity_count = (SELECT MAX(amenity_count) FROM count_per_city)

-- Solution 2: cleaner to remember that JSON table and nice trick
-- number of amenities is one more than number of commas:
WITH city_amenities AS (SELECT city, 
     SUM(LENGTH(amenities) - LENGTH(REPLACE(amenities, ',', '')) + 1) AS amenity_count
                        FROM airbnb_search_details 
                        GROUP BY city)
SELECT city
FROM city_amenities
WHERE amenity_count = (SELECT MAX(amenity_count)FROM city_amenities);
\end{lstlisting}
\end{exm}
