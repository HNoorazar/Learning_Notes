...
It is a good idea to not overlook at basics.
It will saves us time and effort if we do the basics
at the beginning.
We need to know distributions (normal, $t-$, $F-$, Gamma),
Gamma function, expected value and variance and their properties.
Learn the Lagrange Multipliers method. Get a good grasp of moment
...
applied to a sample, is commonly represented by 
$r_{xy}$ and may be referred to as the sample correlation 
coefficient or the sample Pearson correlation coefficient. 
...
\begin{deff}
The correlation matrix of $n$ random variables $X_1, \ldots , X_n$ is the $n \times n$ matrix whose $(i, j)$ entry is $corr(X_i,X_j)$. 
Thus the diagonal entries are all identically unity.
\end{deff}
So, if we have a data sample that are centered and
scaled so that each column of $X$ has norm 1, then
the correlation matrix is given by $R_{XX} = X^TX$.

Let me mention the following quote 
from~\citep{seber2012linear}
that will be mentioned again in~\cref{sec:Hypothesis-Testing}
about correlation coefficient
``Although $1 - r^2$ is a useful measure of fit, 
the correlation $r$ itself is of doubtful use in
making inferences. Tukey [1954] makes 
the provocative but not unreasonable 
statement that correlation coefficients are 
justified in two and only two circumstances, 
when they are regression coefficients, or 
when the measurement of one or both variables on 
a determinate scale is hopeless.''
