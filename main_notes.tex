...
\usepackage{multirow,multicol} % needed for cline for example
...
\newcommand{\pgreen}{\color{titlered}}
...
\def\code#1{{\color{titlered}{\texttt{#1}}}} %Cerulean
\def\vari#1{{\color{Purple}{\texttt{#1}}}} %Cerulean
\def\folder#1{\scriptsize{\colorbox{lightgray}{\texttt{#1}}}}
...
%  language=SQL,
  ...
  morekeywords={SELECT, INSERT, UPDATE, DELETE, FROM, WHERE, AND, OR},  % Additional SQL keywords
  numbers=left,                   % Line numbers on the left
  numberstyle=\tiny\color{gray},   % Style for line numbers
  stepnumber=1,                   % Number every line
  numbersep=5pt,                  % Distance from the code to the line numbers
  frame=single,                   % Draw a frame around the code
  backgroundcolor=\color{lightgray!20},  % Background color of the code
  showstringspaces=false          % Do not highlight spaces in strings
}


% \makeatletter
% \let\c@exm\c@thm  % share the counter
% \makeatother

\crefname{thm}{theorem}{theorem}
\Crefname{thm}{Theorem}{Theorem}
\crefname{deff}{Definition}{Definitions}
\crefname{rem}{Remark}{Remarks}
\crefname{lem}{Lemma}{Lemmas}
\crefname{Coro}{Corollary}{Corollaries}
\crefname{exm}{example}{examples}
\Crefname{exm}{Example}{Examples}
% examples are sharing label counts by theorems. 
% The following line helps LaTex to show proper references
%
\crefalias{exm}{exm}
\crefalias{deff}{deff}
\crefalias{rem}{rem}
...
\usepackage{fontawesome5} % lightbulb
\newcommand{\idea}{{\Large \faLightbulb~}}
\newcommand{\Rq}{{\Large\faSearch~}}
...
Said differently, I do not intend to repeat others' work/words.
Said differently, there are too many books/sources out there with the same content AND the same language;
finding a good/different source is difficult. 
In other words, there are books that have
nothing new to say, yet, they are out there.
I think ideally, the book/paper must have something new to say,
or say something old in a new way that can reach the reader from
a different angle; some people speak the same language, and some do not.

In summary, if you are reading this, I do not intend to add to the noise,
and hopefully, you can find these notes be useful.
...
\input{./05_SVM_main}
\input{./06_practical_issues_main}
\input{./07_tests}
\input{./08_Metrics_main}
\input{./09_model_inference_main}
\input{./10_ML_system_Design_main}
\input{./11_TDA_main}
\input{./12_wavelet_fourier_main}
\input{./13_signals_main}
\input{./14_deep_learning_main}
\input{./15_papers_main}
\input{./20_SQL}
