...
\end{equation}
For $\hat \beta_j$, $z_j = \frac{\hat \beta_j - 0}{\hat \sigma \sqrt{v_j}}$ where
...
Similarly\marginnote{assuming errors $\varepsilon_i$ for each observation have 
...
\end{equation}
The $\sigma$ can be estimated bby \emph{residual standard error} $RSE = \sqrt{\frac{RSS}{N-2}}$.
...
equal to |t| or larger in absolute value, assuming $\beta_1=0$. We call 
...
\begin{equation}
RSE = \sqrt{\frac{1}{N-2} RSS} = \sqrt{\frac{1}{N-2}  \sum(y_i - \hat y_i)}
\end{equation}
...
On the other hand, if $\hat y_i$ is very far from $y_i$ for one or more observations, 
...
See 77-83 of Introduction to Statistical Learning\cite{james2013introduction} if you want.
\end{enumerate}
\end{description}
...
residual plot since there are multiple predictors, we 
...
residuals may have similar values. Figure~\ref{fig:ErrCorrelation} provides an 
illustration. In the top panel, we see the residuals from a linear 
regression fit to data generated with uncorrelated errors. 
...
%  \centering
  ...
  \caption[][-14\baselineskip]{Plots of residuals from simulated time series data sets generated with differing levels of correlation $\rho$ between error terms for adjacent time points.}%
...
%  \centering
  ...
  \caption[][-14\baselineskip]{Left: Observation 41 is a high leverage point, 
...
instead of plotting the residuals, we can plot the studentized 
...
%  \centering
  ...
  \caption[][-14\baselineskip]{Left: Observation 41 is a 
...
splittings. Trees have high variance and low bias.
