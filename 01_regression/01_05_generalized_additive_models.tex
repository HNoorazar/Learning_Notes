...
\end{equation}
\end{deff}{}
\noindent For the first $df$ definition they say that both $\lambda$ and
...
(At this point I do not know about the different purposes!)
...
For a general (non-linear) smoother $df^{err} (\lambda) = N - E[\sum_{i=1}^N y_i^2 - RSS(\lambda)]$.
...
\end{equation}
For a general (non-linear) smoother $df^{var} (\lambda) = \frac{\sum_{i=1}^N var[\hat f_\lambda(x_i)] }{\sigma^2}$.
