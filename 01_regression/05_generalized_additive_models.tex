\chapter{Regression: Generalized Additive Models}
\label{chap:Generalized-Additive-Models}
...
\section{Fitting Additive Models}
...
effects in a flexible way. For concreteness\sidenote{what the hell does correctness mean?} we use as our 
...
\section{Smoothing}
\label{sec:Smoothing}
A\sidenote{From this point on most thing in this chapter
...
is done in neighborhoods and decisions have to be made\sidenote{The book makes this statement about scatterplot smoothing (i.e. single variable case)! I do not know why.}:
\begin{enumerate}
...
$var(\varepsilon) = \sigma^2$\sidenote{The assumption of constant variance 
...
In case of cubic smoothing splines\sidenote{from now on referred to just 
...
Mean squared-error is defined at one
single point (at least in this book) 
$E[(\hat f_{\lambda}(x_i) - f(x_i))^2 ]$,
and the averages over different $x_i$'s is called 
average mean-squared error. But, in different
books/contexts people refer to \textit{average mean-squared error}
by \textit{mean squared-error}.
...
\end{deff}{}
In $PSE$, $Y_i^*$ is a new observation. We are measuring how well
the model predicts the new observations. This is
where (originally leave-one-out) cross-validation comes into 
the picture and tries to estimate $PSE$.
\begin{deff}{}
...
\item [Graphical Methods] Sometimes the performance of 
cross-validation is questionable. \hl{Graphical methods}
along degrees of freedom can be used to choose proper smoothing parameters.
Graphical methods: residual plots, degrees of freedom and 
confidence intervals. ``The ideas for local regression are similar to 
those used in parametric models. Other books on regression 
analysis cover these topics in greater detail than we do here; 
see, for example, chapter 3 of Draper and Smith (1981) 
or chapters 4, 5 and 6 of Myers (1990). Cleveland (1993) is 
a particularly good reference for graphical diagnostics.''\citep{loader2006local}. 
Analytical tools, e.g. $C_p$ or CV can be used to see which 
features are real and which ones are noise, but they cannot be 
used to determine bandwidths (for local regression) for 
(dataset, question) pair!~\citep{loader2006local}.
It is important to remember that no one diagnostic technique 
will explain the whole story of a dataset. Rather, using a combination 
of diagnostic tools and looking at these in conjunction with 
both the fitted curves and the original data provide insight into the data. 
What features are real; have these been adequately modeled; 
are underlying assumptions, such as homogeneity of variance, 
satisfied?\cite{loader2006local}.

\end{description}

\section{Degrees of Freedom of a Smoother}
