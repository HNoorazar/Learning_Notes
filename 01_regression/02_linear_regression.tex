...
Denote that solution by $\hat {\bm \beta}_{LS}$. 
Then, since $\bm y - \bm X \hat{\bm \beta}_{LS}$ is orthogonal to 
...
In matrix notation $\bm X^T (\bm y - \bm X \hat{\bm \beta}_{LS}) = 0$.
Therefore, 
\[
...
normality of errors comes into the picture.
\end{tcolorbox}
...
notation is $\hat f(X)$\dots statisticians and their notations, again! }\fi
...
Beginners' books can be~\cite{frost2020hypothesis}
...
To understand t-tests and F-tests
\begin{enumerate}
\setlength{\itemsep}{-2pt}
\item Start with independent normal variables.
\item Derive the sample mean.
\item Derive the sample variance.
\item Prove the sample mean and sample variance are independent.
\item Show
\begin{itemize}
\setlength{\itemsep}{-2pt}
\item scaled variance follows a chi-square distribution,
\item ratio of a normal variable to the square root of an independent chi-square variable gives a t-distribution,
\item ratio of two independent chi-square variables gives an F-distribution.
\end{itemize}
\end{enumerate}
After that, things (confidence intervals, t-tests, ANOVA, linear regression) follow naturally.
This is exactly how Casella \& Berger develops the subject.
\end{tcolorbox}
...
Seems that \citep{lehmann2005testing} is a well-known one
for theory. Applied and intermediate books that covers hypothesis 
testing are also available~\citep{casella2024statistical,montgomery2017design,wackerly2008mathematical}.

Let us start with definition of p-value. 
\begin{deff}
P-value is measure of how unexpected an observation is under the null hypothesis.
More formally, in null hypothesis significance testing, the $p$-value is the probability 
...
Keep in mind that $\text{p-value} = p(\text{observed value or more extreme} | H_0)$
and NOT: $\text{p-value} = p(H_0 | \text{observed value})$.\\

A very small $p$-value means that such an extreme observed 
...
Kohavi\cite{kohaviABBuster} mentions several examples where
p-value and confidence intervals are misunderstood by people.
Georgiev\cite{georgiev2019} also have some examples in it.
I find Georgiev hard to follow and lots of problems in that book
are presented without addressing them or building an intuition for them.
Mostly solutions are deferred to later chapters which I have 
not arrived to at this point (read 4 chapters so far).
It seems that \citep{kohavi2020trustworthy} is well-written, judging by
reading only 1 page out of it. I have ordered that book.
For experimentation, the following resources looks good,
at least on the paper; \citep{montgomery2017design,box2005statistics}
and the followings seems to be good compliments to the aforementioned ones
with their roots in theory~\citep{wackerly2008mathematical,lehmann2005testing}.

\begin{tcolorbox}[colback=aliceblue!10!white,colframe=blue!70!black,title=\textbf{Remark}]
% olback=green!10!white,colframe=green!80!black
Incorrect definitions from \emph{A/B Testing Intuition Busters
Common Misunderstandings in Online Controlled Experiments}

\begin{itemize}
\setlength{\itemsep}{-2pt}
\item a statistically significant result
with p-value 0.05 has a 5\% chance of being a false positive

\item \dots to determine whether your results are statistically significant: how
confident you can be that the results actually reflect a change in your
visitors' behavior, not just noise or randomness… In statistical terms,
it's 1-[p-value]. If you set a significance threshold of 90\% \dots you can
expect a 10\% error rate

\item we can compute the probability that our observed difference (-0.007)
is due to random chance. This value, called the p-value

\item when statistical significance (that is, it's unlikely the test results are
due to chance) has been achieved
\end{itemize}
...
\centering
\begin{tabular}{cc|cc}
 &  & \multicolumn{2}{c}{\textcolor{blue}{\textbf{Truth}}}  \\
 &  & $H_0$ & $H_a$ \\
\hline
\multirow{2}{*}{\textcolor{blue}{\textbf{Decision}}}
 & Reject $H_0$ & Type I Error & Correct \\
 & Do Not Reject $H_0$ & Correct & Type II Error \\
\end{tabular}
\caption{A summary of the possible scenarios associated with testing the
null hypothesis $H_0$. Type I errors are also known as false positives, and Type II
...
See page 82 of \emph{Think Stat}~\citep{downey2011think}  and do the Exercises.
}
...
\vspace{.15in}
\begin{deff}
The power of the hypothesis test is the probability of correctly rejecting a false $H_0$. 
``Power of a statistical test is the probability of observing a statically significant 
outcome if a true effect of a certain magnitude exist''\cite{georgiev2019}.
...
there is a difference)''\cite{kohavi2020trustworthy}.

\begin{gather} % gather and aligned leads to having one label for eq.
\begin{aligned}
\text{power} &=  1 - P(\text{Type II error})\\
                   &= 1 - \beta \\
                   &=  \frac{\text{number of false nulls that we reject}}{\text{number of false nulls}}\\
\end{aligned}
\end{gather} 
\end{deff}

Grain of salt:
If proof in courtroom has a high bar, then fewer innocent people are convicted (lower Type-I), and more of guilty people are not convicted as well (higher Type-II).
Fixing sample size $\rightarrow$ trade-off between $\alpha$ and $\beta$
If your sample size is fixed, lowering $\alpha$ (being more strict about rejecting $H_0$ reduces false positives, but increases $\beta$ (more false negatives).
Increasing $\alpha$ (being more willing to reject ), reduces $\beta$, but increases Type I errors.
So: you can’t minimize both at the same time without changing something else.
If you increase sample size, you can reduce both $\alpha$ and $\beta$ simultaneously.\\

Here $\alpha$ is the significance level we chose before
the test even is performed. It is also the probability of Type-I error.
It is set by us and has nothing to do with sample size.
Type-II error ($\beta$, probability of failing to reject $H_0$ when it is false)
is affected by sample size. 
Larger samples reduce variability, making it easier to detect a true effect; $\beta$ decreases, power increases. The sample size helps because standard error is a function of sample
size, they are inversely related. 
Sample size changes during sequential sampling, for example.
You can collect enough data to reach a good power, but that needs
to be done with care. Dig deeper later.

\noindent{\bf Sample size for a test}
\begin{description}
\item [Significance Level] $\alpha$
\item [Power] $p=1-\beta$ . Power is the probability of correctly rejecting the null hypothesis when it is false. 
A commonly targeted power is 0.8 (or 80\%), meaning there’s an 80\% chance of detecting a true effect if it exists. 
Higher power requires a larger sample size.
Type II error ($\beta$) is the failure to reject the null hypothesis when it should be rejected. 
If you have a power of 0.8, $\beta = 0.2$
\item [Effect Size] The effect size is the magnitude of the difference you want to detect in the experiment. Larger effect sizes are easier to detect, meaning you need fewer participants. Small effect sizes require larger sample sizes to ensure the results are statistically significant.
Common effect size metrics include Cohen's d for t-tests or odds ratios for categorical outcomes.
\item [Variance or Standard Deviation] The more variable the data, the larger the sample size required to detect a meaningful effect. If the data is more spread out, you need more data points to detect a significant difference between groups.
\item [Design of the Experiment] \hl{Whether you are doing a t-test, ANOVA, regression, or chi-square test, the statistical test impacts the sample size. For example, calculating sample size for a paired t-test differs from that for an independent two-sample t-test.}
\end{description}

For a t-test: You can use the formula for sample size per group calculation for a two-sample t-test:
\begin{equation}
n = \frac{(Z_{\alpha/2} + Z_\beta)^2 \times (2 \sigma^2)}{\Delta^2},
\end{equation}
where $\Delta$ is minimum effect size. It is the absolute MDE ($p_2 - p_1$),
\hl{but there is more to it. Do more research. For example, how to apply it on revenue lift.}

\hl{The following is a mess, we need to open it up. why}
sometimes $\sigma^2 = p1 (1-p1) + p2(1-p2)$ and sometimes
$\sigma^2 = p1 (1-p1)/n1 + p2(1-p2)/n2$.
In the Bernouli/proportion situation the $\sigma^2 = p (1-p)$.
If there are two populations then $\sigma^2 = p_1 (1-p_1) + p_2 (1-p_2)$.
Pooled variance assumes $p_1 = p_2$ under null hypothesis, and
then 
\[p_{\text{pooled}} = \frac{n_1*p_1 + n_2*p_2}{n_1 + n_2}\]
and if size of each group is identical, then the pooled variance is just 
$p_{\text{pooled}} = \frac{p_1 + n_2}{2}$ and then $\sigma_{\text{pooled}} = p_{\text{pooled}} * (1 - p_{\text{pooled}}) * (\frac{1}{_n1} + \frac{1}{n_2})$.

If $\Delta$ (which is raw) is replaced by Cohen's $d$ (for t-test) then the
variance in numerator will disappear:
\[d = \frac{\mu_1 - \mu_2}{\sigma}\]


\begin{exm}
Say baseline conversion is 10\%, I want to detect a 2 percentage point lift (to 12\%), with $\alpha = 0.05$ and 
80\% power.
\[
n = \frac{2 \times (1.96 + 0.84)^2 \times 0.1 \times .9}{0.02^2} = 3528
\]
3,528 per group, so, in total, almost 7,000 samples.
\end{exm}


From~\citep{james2013introduction} ``Once the hypothesis test is performed, the row of the
table is known (based on whether or not we have rejected $H_0$); 
however, it is impossible for us to know which column we are in. 
If we reject $H_0$ when $H_0$ is false (i.e., when $H_a$ is true), 
or if we do not reject $H_0$ when it is true, then we arrived at the correct result. 
However, if we erroneously reject $H_0$ when $H_0$ is in fact true, then we have committed a Type I error. 
The Type I error rate is defined as the probability of making a Type I error given that Type I error $H_0$ holds, i.e., the probability of incorrectly rejecting $H_0$. 
Alternatively, if rate we do not reject $H_0$ when $H_0$ is in fact false, then we have committed a
Type II error.''\marginnote{Read page 563 of ISPL.}\\

Statistical power of a test is characteristic of a test procedure and 
the predetermined sample size and the predetermined significance level
of a certain null hypothesis. Power makes no use of collected data,
its calculation is hypothetical, and is of no use after a test is run\cite{georgiev2019}.
A good book that covers power is written by Cohen\cite{cohen2013statistical}.


Let us look at regression examples.
...
Multiple testing is more challenging than a single hypothesis test.
But now suppose that we wish to test $m$ null hypotheses,
$H_{01}, \dots, H_{0m}$. 
Will it do to simply reject all null hypotheses for which the
...
From~\citep{james2013introduction}:
``Suppose that we flip 1,024 fair
coins ten times each. Then we would expect (on average) one coin to
...
is fair would lead to a p-value below 0.002! But it would be incorrect to
...
So, we need to control for a family-wise error rate.
If we have one hypothesis, we use $\alpha$, significance level, to control
for Type I error; $\alpha$ is probability of rejecting null hypothesis by mistake, i.e.
$\alpha = p(\text{falsely rejecting }H_0)$. 
Family-wise error rate is probability of making at least one such mistake when we have multiple hypothesis. Using notation of~\cref{tab:fwer1} $\text{FWER} = P(V \ge 1)$.
\begin{table}[h]
\label{tab:fwer1}
\centering
\begin{tabular}{cc|cc|c}
 &  & \multicolumn{2}{c}{\textcolor{blue}{\textbf{Truth}}}  & Total\\
 &  & $H_0$ & $H_a$ \\
\hline
\multirow{2}{*}{\textcolor{blue}{\textbf{Decision}}}
 & Reject $H_0$ & V & S & R\\
...
& & $m_0$ & $m - m_0$ & m
\end{tabular}
\caption{A summary of the results of testing m null hypotheses.
A given null hypothesis is either true or false, and a test of that null hypothesis can either
reject or fail to reject it. 
In practice, the individual values of $V$ , $S$, $U$, and $W$ are unknown. 
However, we do have access to $V + S = R$ and $U+W = m-R$, which are
the numbers of null hypotheses rejected and not rejected, respectively.}
\end{table}

\vspace{.2in}
\noindent {\color{blue}\bf Approaches to Control the Family-Wise Error Rate}\\
Bonferroni method and Holm’s step-down procedure are very general-purpose 
approaches for controlling the FWER that can be applied whenever m p-values have been computed, regardless of the form of the null hypotheses, the choice of test statistics, or the
(in)dependence of the p-values. Tukey’s method and Scheff{\'e}'s method are for 
certain situations, more specialized approaches for controlling the FWER may be
preferable. See~\citep{james2013introduction} page 567.

\begin{align*}
\text{FWER}(\alpha) &= P(V \ge 1)\\
                     &= 1- P(V =0)\\
                     &= 1- \text{P(do not falsely reject any null hypotheses)}\\
                     &= 1 - P(\bigcap_i \text{\{do not falsely reject } H_{0i}  \text{\}})\\
                     &= \text{assuming independence, then,}\\
                     &= 1 - \prod_i (1 - \alpha)\\
                     &= 1 - (1 - \alpha)^m\\
                     &= \text{under independence, or}\\
                     &= P(\text{falsely reject at least one null hypothesis})\\
                     &= P(\cup_i A_i)\\
                     &\le \sum_i P(A_i)
\end{align*}
where $A_i$ is the event that we make a Type I error for the $i$th null hypothesis,
and the inequality comes from $P(A \cup B) \le P(A) + P(B)$.
Bonferroni uses this fact and sets significance level to $\alpha/m$;
\[\text{FWER}(\alpha/m) \le  m \times \frac{\alpha}{m} \]
